...
class MLPBlock(MLP):
    ...
    def forward(self, input, masklayer=None, mask_idx=None):
        idx = 0
        if masklayer is not None:
            for m in self.modules():
                if isinstance(m, nn.Linear):
                    if idx == 1:
                        assert input.shape[-1] == 3072
                        input[..., mask_idx] = 0
                    idx += 1
                input = m(input)
            return input
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor, masklayer=None, mask_idx=None):
        ...
        y = self.ln_2(x)
        y = self.mlp(y, masklayer=masklayer, mask_idx=mask_idx)
        return x + y
    ...
    def forward(self, input: torch.Tensor, masklayer=None, mask_idx=None):
        ...
        if masklayer is None:
            ...
            for layer_idx, layer in enumerate(self.layers):
                if layer_idx == masklayer:
                    out = layer(out, masklayer=masklayer, mask_idx=mask_idx)
                else:
                    out = layer(out)
        return self.ln(out)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, masklayer=None, mask_idx=None):
        ...
        x = self.encoder(x, masklayer=masklayer, mask_idx=mask_idx)
