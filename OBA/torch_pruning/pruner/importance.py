...
class MagnitudeImportance(Importance):
    ...
    def _reduce(self, group_imp: typing.List[torch.Tensor], group_idxs: typing.List[typing.List[int]]):
        ...
        if self.group_reduction == 'prod':
            reduced_imp = torch.ones_like(group_imp[0]).cpu()
        elif self.group_reduction == 'max':
            reduced_imp = torch.ones_like(group_imp[0]).cpu() * -99999
        else:
            reduced_imp = torch.zeros_like(group_imp[0]).cpu()
        reduced_imp = reduced_imp.to(group_imp[0].device)
        for i, (imp, root_idxs) in enumerate(zip(group_imp, group_idxs)):
            if self.group_reduction == "sum" or self.group_reduction == "mean":
                if len(imp.shape) == 2:
                    reduced_imp.scatter_add_(1, torch.tensor(root_idxs, device=imp.device).unsqueeze(0).repeat(imp.shape[0], 1), imp) # accumulated importance
                else:
        ...
        if self.group_reduction == "mean":
            ### exclude zero importance layers
            mean_imp = sum(list(map(lambda x: torch.any(x != 0), group_imp)))
            reduced_imp /= mean_imp if mean_imp != 0 else 1
        reduced_imp = reduced_imp.cpu()
        return reduced_imp
...
class PostLayerTaylorImportance(MagnitudeImportance):
    """First-order taylor expansion of the loss function.
       https://openaccess.thecvf.com/content_CVPR_2019/papers/Molchanov_Importance_Estimation_for_Neural_Network_Pruning_CVPR_2019_paper.pdf
    """
