...
import sys
sys.path.append("/home/wangzefang/Projects/EdgeVAR/Torch-Pruning")
...
def prepare_calibration_d16_last_input(args, model, dataloader,device,token_len):
    ...
    class Catcher(nn.Module):
        ...
        def forward(self, x, **kwargs):  ##ori 只选取最后一层
            ...
            if tokens ==token_len**2:
                # print("当前token长度为:{}".format(tokens))
                cache.append({
                "i": len(cache),
                "x": x,
                "cond_BD": kwargs["cond_BD"],
...
def model_slimming(model, dataloader, args):
    ...
    with torch.no_grad():
        still_cur = bool
        groups = []
        current_group = [args.specific_layer[0]]
        
        for idx in args.specific_layer[1:]:
            if idx == current_group[-1]:
        ...
        groups.append(current_group)
        print(groups)
        for cur in range(len(groups)):
            still_cur = False
            print("cur is : {}".format(cur))
            for p in range(len(groups[cur])):
                print(groups[cur])
                layer_num += 1
                i = layer_num - 1
                print("cur第几种:{},i当前种类内有几个:{}".format(p,len(groups[cur])))

                t1 = time.time()
                
                if still_cur == False:
                    inps, outs, cond_BD, cache = prepare_calibration_d16_last_input(
                        args, model, dataloader, dev, groups[cur][p] )
                    still_cur = True
...
def main(args):
    ...
    vae_ckpt, var_ckpt = '/home/wangzefang/Projects/EdgeVAR/VAR/model_zoo/vae_ch160v4096z32.pth', f'/home/wangzefang/Projects/EdgeVAR/VAR/model_zoo/var_d{MODEL_DEPTH}.pth'
    ...
    save_model = "/home/wangzefang/Projects/EdgeVAR/slimgpt_pub/output/sparsity_model"
