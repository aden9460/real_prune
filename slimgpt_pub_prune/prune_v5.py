    ...
    with torch.no_grad():
        ...
        for i in range(len(layers)):
            ...
            for names in sequential:
                ...
                # cache_layer = cache[i]  # cache 是所有层的列表，每层一个列表
                # print("len cache_layer:{}".format(cache_layer))
                # out_layer = outs[i]     # outs 也是每层一个列表
                class Catcher(nn.Module):
                    def __init__(self, module, layer_idx,batch_idx, caches,outs):
                        super().__init__()
                        self.module = module
                        self.layer_idx = layer_idx
                        self.batch_idx = batch_idx
                        self.caches = caches  # 直接引用外部 caches
                        self.outs = outs  # 直接引用外部outs
                    def forward(self, x, **kwargs):
                        
                        self.caches[self.layer_idx][self.batch_idx] = {
                            "i": self.batch_idx,
                            "x": x,
                            "cond_BD": kwargs.get("cond_BD", None),
                        }
                        y = self.module(x, **kwargs)
                        self.outs[self.layer_idx][self.batch_idx] = {
                            "i": self.batch_idx,
                            "x": y,
                            "cond_BD": kwargs.get("cond_BD", None),
                        }
                ...
                layer = Catcher(layer, layer_idx=i,batch_idx=0,caches=caches, outs=outs)
                try:
                    for batch_idx, batch in enumerate(dataloader):
                        model(batch)
                except ValueError:
