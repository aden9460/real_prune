...
parser.add_argument('--output_dir', type=str, default='fid_samples', help='输出目录')
...
parser.add_argument('--sparsity', type=float, default=0, help='稀疏度')
...
var_ckpt = '/wanghuan/data/wangzefang/slim_VAR_copy/VAR/model_zoo/var_d24.pth'
...
# 创建进度条
progress_bar = tqdm(total=total_images, desc="生成FID样本")
...
for class_id in range(args.total_classes):
    # 按批次生成每个类别的图像
    for batch_start in range(0, args.samples_per_class, batch_size):
        # 计算批次大小（最后一批可能小于batch_size）
        current_batch_size = min(batch_size, args.samples_per_class - batch_start)

        # 准备当前批次
        batch_class_ids = [class_id] * current_batch_size
        batch_indices = list(range(batch_start, batch_start + current_batch_size))

        # 生成当前批次的图像
        generate_images_batch(batch_class_ids, batch_indices, args.seed + class_id * args.samples_per_class)

        # 更新完成的图像数量和进度条
        completed_images += current_batch_size
