...
基于原始train.py修改，添加蒸馏支持。
保持所有原有功能：DDP、progressive training、数据加载等。
"""
...
def build_everything_distill(args: arg_util.Args):
    """扩展build_everything函数，添加蒸馏支持

    基于原train.py的build_everything函数，添加教师模型加载和蒸馏训练器构建。
    """
    # ========== 原有构建逻辑（完全保持不变） ==========

    # resume
    auto_resume_info, start_ep, start_it, trainer_state, args_state = auto_resume(args, 'ar-ckpt-best*.pth')
    ...
    if trainer_state is None or len(trainer_state) == 0:
        ...
        if 'trainer' in checkpoint:
            ...
            if 'var_wo_ddp' in checkpoint['trainer']:
                ...
                print("[学生模型] 警告：checkpoint中未找到var_wo_ddp，尝试直接加载...")
                var_wo_ddp.load_state_dict(checkpoint, strict=True)
        else:
            print("[学生模型] 加载原始模型权重...")
            var_wo_ddp.load_state_dict(checkpoint, strict=True)
    else:
        # 断点续训：跳过初始权重加载，稍后由trainer.load_state_dict恢复
        print(f"[学生模型] 检测到resume checkpoint (epoch {start_ep})，跳过初始权重加载")
        print(f"[学生模型] 模型权重将通过 trainer.load_state_dict 恢复")
    ...
    # ========== 蒸馏特定：加载教师模型 ==========
    teacher_model = None
    if args.enable_distillation and args.teacher_model_path:
        print(f"\\n{'='*60}")
        print("开始加载教师模型...")
        try:
            teacher_model = distill_utils.load_teacher_model(args)
            print("教师模型加载成功")

            # 打印蒸馏配置摘要
            if dist.is_master():
                config_summary = distill_utils.create_distill_config_summary(args)
    ...
    if teacher_model is not None:
        ...
        print(f"[训练器] 使用蒸馏训练器 (DistillationVARTrainer)")
        print(f"[蒸馏模式] {args.distill_type} 蒸馏，权重 {args.distill_alpha}/{args.distill_beta}")
    else:
        trainer = VARTrainer(
            device=args.device, patch_nums=args.patch_nums, resos=args.resos,
            vae_local=vae_local, var_wo_ddp=var_wo_ddp, var=var,
        ...
        print(f"[训练器] 使用普通训练器 (VARTrainer)")

    if trainer_state is not None and len(trainer_state):
        print(f"[Resume] 恢复训练器状态 (epoch {start_ep}, iter {start_it})...")
        trainer.load_state_dict(trainer_state, strict=False, skip_vae=True)  # don't load vae again
        print(f"[Resume] 成功恢复，将从 epoch {start_ep} 继续训练")
    else:
        print(f"[训练器] 使用初始状态，从 epoch 0 开始")
...
def main_distillation_training():
    ...
    基于原train.py的main_training函数，添加蒸馏参数验证。
    """
    ...
    if args.enable_distillation:
        try:
            args.validate_distill_args()
            print("[参数验证] 蒸馏参数验证通过")
        except Exception as e:
            print(f"[参数验证] 蒸馏参数验证失败: {e}")
            print("[参数验证] 将禁用蒸馏，使用普通训练")
            args.enable_distillation = False
    ...
    for ep in range(start_ep, args.ep):
        ...
        # 添加蒸馏损失记录
        if hasattr(trainer, 'get_distill_stats'):
            distill_stats = trainer.get_distill_stats()
            if distill_stats['total_steps'] > 0:
                AR_ep_loss.update({
                    'distill_task_loss': distill_stats['avg_task_loss'],
                    'distill_distill_loss': distill_stats['avg_distill_loss'],
                    'distill_similarity': distill_stats['avg_logits_similarity']
                })

        is_val_and_also_saving = (ep + 1) % args.ckpt_save_interval == 0 or (ep + 1) == args.ep
        if is_val_and_also_saving:
            ...
            if dist.is_local_master():
                # 创建带epoch编号的checkpoint文件名
                local_out_ckpt_ep = os.path.join(args.local_out_dir_path, f'ar-ckpt-ep{ep+1:03d}.pth')
                local_out_ckpt_last = os.path.join(args.local_out_dir_path, 'ar-ckpt-last.pth')
                local_out_ckpt_best = os.path.join(args.local_out_dir_path, 'ar-ckpt-best.pth')

                print(f'[saving ckpt] ep{ep+1} ...', end='', flush=True)
                ...
                # 同时保存为 last.pth（用于断点续训）
                shutil.copy(local_out_ckpt_ep, local_out_ckpt_last)

                # 如果是最佳模型，也保存为 best.pth
                if best_updated:
                    shutil.copy(local_out_ckpt_ep, local_out_ckpt_best)
                    print(f'     [saving ckpt](*) finished!  @ {local_out_ckpt_ep} (best)', flush=True, clean=True)
                else:
                    print(f'     [saving ckpt](*) finished!  @ {local_out_ckpt_ep}', flush=True, clean=True)
            dist.barrier()
    ...
    print('\\n\\n')
    print(f'  [*] [蒸馏训练完成]  总耗时: {total_time},   Lm: {best_L_mean:.3f} ({L_mean}),   Lt: {best_L_tail:.3f} ({L_tail})')

    # 打印蒸馏训练总结
    if hasattr(trainer, 'get_distill_stats'):
        distill_stats = trainer.get_distill_stats()
        if distill_stats['total_steps'] > 0:
            total_steps = distill_stats["total_steps"]
            avg_task_loss = distill_stats["avg_task_loss"]
            avg_distill_loss = distill_stats["avg_distill_loss"]
            avg_similarity = distill_stats["avg_logits_similarity"]

            print(f'  [*] [蒸馏统计]  总步数: {total_steps}')
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger, ld_or_itrt, iters_train: int, trainer):
    ...
    # 添加蒸馏相关指标
    if hasattr(trainer, 'distill_losses'):
        [me.add_meter(x, misc.SmoothedValue(fmt='{median:.4f} ({global_avg:.4f})')) for x in ['TaskL', 'DistillL', 'TotalL']]
