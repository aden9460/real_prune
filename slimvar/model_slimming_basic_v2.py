...
def model_slimming(model, calibration_labels, calibration_tokens, args):
    ...
    # Load selected-heads plan if enabled
    selected_plan = None
    if getattr(args, 'use_selected_heads', False) and getattr(args, 'selected_heads_json', ''):
        try:
            import json as _json
            with open(args.selected_heads_json, 'r') as f:
                _plan = _json.load(f)
            # Map: layer index -> list of pruned head indices
            selected_plan = {int(rec['layer']): rec.get('pruned', []) for rec in _plan.get('layers', [])}
            print(f"\n✓ Loaded selected-heads plan: {args.selected_heads_json}")
        except Exception as e:
            print(f"\n⚠ Failed to load selected heads: {e}")
            selected_plan = None

    # Group-wise compensation is not used; keep selected-heads global compensation only
        ...
        if args.minlayer <= i < args.maxlayer:
            ...
            for names in sequential:
                ...
                for name in prune_order:
                    ...
                    # Pre-fetch original in_features before any TP modifications
                    if name == "attn.proj":
                        _tl = get_module_by_name(model.blocks[i], name)
                        old_in_features = _tl.in_features

                    if name == "attn.proj" and selected_plan is not None and i in selected_plan:
                        pruned_heads = selected_plan.get(i, [])
                        print(f"  Layer {i}: attn.proj - using selected heads ({len(pruned_heads)} heads)")
                        # Map head indices -> column indices
                        target_layer = get_module_by_name(model.blocks[i], name)
                        head_dim = old_in_features // model.blocks[i].attn.num_heads
                        prune_cols = []
                        for h in pruned_heads:
                            start = h * head_dim
                            prune_cols.extend(range(start, start + head_dim))
                        prune_cols = torch.tensor(prune_cols, dtype=torch.long, device=device)
                        # Iterative compensation over selected heads (default: 1 head per step)
                        heads_per_step = getattr(args, 'selected_iter_heads_per_step', 1)
                        chunk_size = max(1, int(heads_per_step) * head_dim)
                        idx = pruner_dict[name].struct_prune_with_indices_iterative(
                            prune_columns=prune_cols,
                    ...
                    if name == "ffn.fc2":
                        ...
                        # Update num_heads based on actually removed heads (robust to selected-heads mode)
                        ...
                        # Update scale parameter - Preserve original learned values
                        head_dim = old_in_features // model.blocks[i].attn.num_heads
                        old_num_heads = old_in_features // head_dim
                        ...
                        # Update attn.num_heads to actual kept heads
                        model.blocks[i].attn.num_heads = int(len(keep_heads))
                        ...
                        # Use the original hidden size (before pruning) for Q/K/V offsets
                        hidden = old_in_features
...
if __name__ == "__main__":
    ...
    # Selected-heads guided pruning for attn.proj
    parser.add_argument(
        "--use_selected_heads", action="store_true",
        help="Enable selected-heads guided pruning for attn.proj (global compensation on the rest)"
    )
    parser.add_argument(
        "--selected_heads_json", type=str, default="",
        help="Path to selected-heads plan JSON (e.g., head_sharpness_images/pruning_plan_40pct.json)"
    )
    parser.add_argument(
        "--selected_iter_heads_per_step", type=int, default=1,
        help="Iterative compensation for selected heads: number of heads per iteration (default: 1)"
    )

    # (Group-wise compensation removed; using global compensation only)
