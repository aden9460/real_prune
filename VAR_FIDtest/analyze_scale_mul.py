...
def load_var_model(model_depth, vae_ckpt_path, var_ckpt_path, sparsity=0.0, device='cuda'):
    """加载VAR模型"""
    from models import build_vae_var

    patch_nums = (1, 2, 3, 4, 5, 6, 8, 10, 13, 16)

    # 创建 args 对象传递给 build_vae_var
    class SimpleArgs:
        def __init__(self):
    ...
    # 加载 VAR checkpoint (支持训练检查点格式)
    checkpoint = torch.load(var_ckpt_path, map_location='cpu')

    if 'trainer' in checkpoint:
        print("  检测到训练检查点格式，正在提取模型权重...")
        if 'var_wo_ddp' in checkpoint['trainer']:
            model_weights = checkpoint['trainer']['var_wo_ddp']
            var.load_state_dict(model_weights, strict=False)
    ...
    print(f'✓ VAR-d{model_depth} model loaded successfully (sparsity={sparsity}).')
    return vae, var
...
def extract_scale_mul(model):
    ...
    直接从模型中读取实际存在的head数量，无需通过sparsity计算
    ...
    print(f"  Detected {num_heads} heads in the model")
    ...
    return scale_mul_matrix, scale_mul_raw, num_heads
...
def visualize_scale_mul(scale_mul_matrix, stats, output_dir):
    ...
    # 自适应高度：根据head数量调整
    fig_height = max(6, min(10, num_heads * 0.3))
...
def main(args):
    ...
    vae, var = load_var_model(args.model_depth, vae_ckpt, var_ckpt, sparsity=args.sparsity, device='cpu')

    # 2. 提取scale_mul
    print("\nStep 2: Extracting scale_mul parameters...")
    scale_mul_matrix, scale_mul_raw, num_heads = extract_scale_mul(var)
    ...
    config = {
        ...
        'sparsity': args.sparsity,  # 用户提供的sparsity（仅作为元数据记录）
        'num_heads': int(num_heads),  # 模型中实际检测到的head数量
        'vae_ckpt': vae_ckpt,
        'var_ckpt': var_ckpt,
    ...
    print(f"✓ Saved: {args.output_dir}/config.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Sparsity ratio for pruned models (default: 0.0 for unpruned models)"
    )
